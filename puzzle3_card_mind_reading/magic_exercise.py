...
def outputFirstCard(numbers, oneTwo, cards):
    ...
    # From the numbers of the same suite, do the encoding.
    # Circular modulation, mod 13 to determine the circular relation
    encode = (numbers[oneTwo[0]] - numbers[oneTwo[1]]) % 13
    # The circular distance between the two cards
    if encode > 0 and encode <= 6:
        # Encode in a clockwise direction
        hidden = oneTwo[0]
        other = oneTwo[1]
    else:
        # Encode in a counter-clockwise direction
        hidden = oneTwo[1]
        other = oneTwo[0]
...
def outputNext3Cards(code, ind):
    # Encoding for all possible cases with the sorted remaining cards.
    # Here, CDHS encoding is used. Club, Diamond, Hearts and spades
    if code == 1:
...
def ComputerAssistant():
    ...
    # A suit array is declared since we only have CDHS
    numsuits = [0, 0, 0, 0]
    number = 0
    while number < 99999:
    ...
    for i in range(5):
        n = clist[i]
        # From the deck you just fund
        cards.append(deck[n])
        cind.append(n)
        # This gives suite of card
        cardsuits.append(n % 4)
        # Since we have 52 numbers representing the deck, this gives the number of card
        cnumbers.append(n // 4)
        numsuits[n % 4] += 1
        # Check if the given suite already exists. Due to piegenhole principal, there must be
        # same suite within this 5 drawn cards.
        if numsuits[n % 4] > 1:
            pairsuit = n % 4

    # Add this pair suit into card pairs, one of it would be chosen as the first card.
    cardh = []
    for i in range(5):
        if cardsuits[i] == pairsuit:
            cardh.append(i)

    # Given a pairs of card with same suite, generate the first card.
    hidden, other, encode = outputFirstCard(cnumbers, cardh, cards)

    # The remaining cards except the encoded pairs.
    remindices = []
    for i in range(5):
        if i != hidden and i != other:
            remindices.append(cind[i])

    # Sort these cards in order
    sortList(remindices)
    # Rearrange them according to the distance of the pair.
