...
def bestTimeToPartySmart(schedule):
    ...
    times = []
    # Since we are going to compute the start and end time in different way. Treat them as different element
    # If starting, +1 if end -1 otherwise do nothing.
    for c in schedule:
...
def chooseTime(times):
    ...
    # The current count, density of celebrities
    rcount = 0
    # The max density of celebrities keep comparing
    # Whenever the current density is larger, change the max density with the new one.
    maxcount = 0
